  ...
  if (pba->has_fk == _TRUE_) {
   ...
   double T_tilde_fk =ppw->pvecmetric[ppw->index_mt_T_tilde_fk];
	  ...
	  *metric_euler/k2*ppt->ALPHA_fk
      ...
      double K2_fk = k2/(a_prime_over_a*a_prime_over_a);
      double Omega_fk = pvecback[pba->index_bg_Omega_fk];
      double Omega_m = 1. - ppw->pvecback[pba->index_bg_Omega_fk];
    
      // F(K) attractor
      double delta_fk_AIC = (2./3.*K2_fk*ppw->pvecmetric[ppw->index_mt_Z_tilde_fk]*ppt->ALPHA_fk)/((ppw->pvecback[pba->index_bg_cPDDE]*Omega_m)/(ppw->pvecback[pba->index_bg_cPDM])-Omega_fk);

      double theta_fk_AIC = 0.;

      /*if (a<pba->a_start_fk){
	dy[pv->index_pt_delta_tilde_fk] = 0.;
	dy[pv->index_pt_theta_tilde_fk] = 0.;

          y[pv->index_pt_delta_tilde_fk] = delta_fk_AIC;
          y[pv->index_pt_theta_tilde_fk] = theta_fk_AIC;
	                                                      }*/

      /*double criterion = 1.-y[pv->index_pt_delta_tilde_fk]/delta_fk_AIC;

      if (a>pba->a_start_fk && criterion>0.01){
	dy[pv->index_pt_delta_tilde_fk] = 0.;
	dy[pv->index_pt_theta_tilde_fk] = 0.;

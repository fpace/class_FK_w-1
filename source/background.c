...
int background_functions(
  ...
  if (pba->has_fk == _TRUE_) {
   ...
   /*else if(pvecback[pba->index_bg_a] > pba->a_start_fk) {
	pvecback[pba->index_bg_cPD] = exp(pba->P1);
	pvecback[pba->index_bg_cPT] = ((pba->P2)/(2.0*(pba->P2)+1.0+2.0*pvecback[pba->index_bg_gamma2]))*(1.0-2.0*(pvecback[pba->index_bg_epsilon_H]*pvecback[pba->index_bg_gamma2]+exp(pba->P1)));
	pvecback[pba->index_bg_cPX] = -1.0*pvecback[pba->index_bg_cPT]*2.0*pvecback[pba->index_bg_gamma1]*(1.0+2.0*pvecback[pba->index_bg_gamma2])/(2.0*pvecback[pba->index_bg_gamma1]-1.0);
	pvecback[pba->index_bg_cPY] = 2.0/3.0*(pba->P2)*pvecback[pba->index_bg_gamma1]/(1.0-2.0*pvecback[pba->index_bg_gamma1]);

	pvecback[pba->index_bg_cGD] = (exp(pba->P1)/3.0+(pba->P3))*(1.0+2.0*pvecback[pba->index_bg_gamma2])- pba->w_fk;
	pvecback[pba->index_bg_cGT] = 1.0/(3.0*(2.0*(pba->P2)+1.0+2.0*pvecback[pba->index_bg_gamma2]))*((1.0-2.0*(exp(pba->P1)))*(1.0+2.0*pvecback[pba->index_bg_gamma2])-6.0*pvecback[pba->index_bg_epsilon_H]*pvecback[pba->index_bg_gamma2]*(1.0+2.0/3.0*pvecback[pba->index_bg_gamma3])) + pba->w_fk;
	pvecback[pba->index_bg_cGW] = (1.0+pba->w_fk)/pvecback[pba->index_bg_epsilon_H];
	pvecback[pba->index_bg_cGX] = 4.0/(3.0*(2.0*(pba->P2)+1.0+2.0*pvecback[pba->index_bg_gamma2]))*pvecback[pba->index_bg_gamma1]/(2.0*pvecback[pba->index_bg_gamma1]-1.0)*((1.0+(exp(pba->P1)))*pow((1.0+2.0*pvecback[pba->index_bg_gamma2]),2.)+3.0*(pba->P2)*(1.0+2.0*pvecback[pba->index_bg_gamma2]*(1.0-pvecback[pba->index_bg_epsilon_H]*(1.0+2.0/3.0*pvecback[pba->index_bg_gamma3]))));
	pvecback[pba->index_bg_cGY] = (-1.0/3.0)* (1.0+pba->w_fk)/pvecback[pba->index_bg_epsilon_H];
   }*/

      pvecback[pba->index_bg_cPDDE] = 0.0;
      pvecback[pba->index_bg_cPDM] = 0.0;
      pvecback[pba->index_bg_cPTDE] = 0.0;
      pvecback[pba->index_bg_cPTM] = 0.0;
      
      pvecback[pba->index_bg_cGDDE] = 1.2;
      pvecback[pba->index_bg_cGDM] =  0.01 - pvecback[pba->index_bg_Omega_fk]*0.025;
      pvecback[pba->index_bg_cGTDE] = 0.0;
      pvecback[pba->index_bg_cGTM] = 0.0;
